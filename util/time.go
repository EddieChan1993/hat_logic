...
// refreshAt 最新刷新时间
// MXSec 当前差值/单位时间
func RefreshCd(XSec, refreshAt int64, do func(MXSec int64)) (nowRefreshAt, cd int64) {
	now := TimeNowUnix()
	if refreshAt == 0 {
		do(0)
		refreshAt = now
...
// TimeCd 获取cd剩余时间
func TimeCd(XSec, refreshAt int64) (cd int32) {
	if refreshAt == 0 {
		return 0
	}
	now := TimeNowUnix()
