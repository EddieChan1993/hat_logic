...
// IRoleHandleMod 角色模块事件
type IRoleHandleMod interface {
	OnlineHandle()  //上线事件
	OfflineHandle() //离线事件
	ReqHandle()     //任意请求事件
}
...
type IModDb interface {
	IModBase
	InitDbRoleData(db *pbdb.RoleData) //初始化db参数，用于数据解析（注意：该方法下不能调用其他模块，且不能给slice分配长度，否则会被proto进行 长度累加）
...
func (this_ *TModMgr) isRoleMod(modIns IModBase) (roleModIns IRoleHandleMod, isOk bool) {
	roleModIns, isOk = modIns.(IRoleHandleMod)
	return
}
